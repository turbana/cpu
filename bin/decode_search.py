...
def load_config(filename):
    ...
    header = reader.next()
    for row in reader:
        table.append(dict((header[i], parse_value(row[i]))
...
class SearchState(object):
    ...
    def solve(self):
        ...
        proc = subprocess.Popen(ESPRESSO, stdin=subprocess.PIPE,
    ...
    def truth_table(self):
        table = {
            ...
            "outputs": [key for key in self.config[0]
                        if key != "Inst" and key not in self.muxes],
            "data": []
        }
    ...
    def dump(self, stream):
        ...
        for mux, choices in self.muxes.items():
            ...
            pretty_choices = ",".join("%d=%s" % (i, clean(val))
...
class MetaState(object):
    ...
    def __init__(self, config, seed, options):
        ...
        f = lambda temp, _: temp * options["_cool_mult"]
        self.options["cool_func"] = f
        self._score = None
    ...
    def neighbor(self):
        ...
        is_num = isinstance(MetaState.RANGES[key][0], int)
        rand = random.randint if is_num else random.uniform
...
def sim_anneal(state, options, emit=None, emit_mod=1):
    ...
    while settle != settle_at and iterations != max_iterations:
        ...
        if emit and iterations % emit_mod == 0:
            emit("\rscore=%4d temp=%0.2f iterations=%d settle=%2d    " % (
                state.score, temp, iterations, settle))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Searches for efficient "
                                     "encoding of opcodes and decode stage")
    add = parser.add_argument
    add("iterations", type=int,
        help="Stop after settling on a solution for ITERATIONS")
    add("-q", "--quiet", action="store_true", help="Suppress output")
    add("-m", "--max-iterations", type=int, default=-1,
        help="Maximum iterations to search")
    add("-s", "--seed", type=int, default=None,
        help="Random number generator seed")
    add("--meta-anneal", action="store_true",
        help="Perform a meta search to find optimal annealing parameters")
