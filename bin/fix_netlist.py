...
Fixes verilog code generated by the netlister.
  ...
  4. Set module parameters (for multi-schematics: inputs that match outputs are
     not generated as module parameters)
  5. Ensure the width of wires match the module input width (the netlister only
     generates wires to the width that it needs to generate the circuit; if we
     use fewer wires than the input is declared as we get a mis-match)
"""
...
def parse_wires(schems):
    wires = set()
    devices = {"IPAD": "input", "IOPAD": "inout", "OPAD": "output"}
    def clean_name(pad):
        name = pad.netlabel
    ...
    for name, widths in itertools.groupby(wire_list, key_name):
        widths = list(widths)
        if len(widths) > 1:
            raise NetlistException("Width mis-match found for %s: %s" % (
                name, ", ".join(map(key_width, widths))))
    return wires
